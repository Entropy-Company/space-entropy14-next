...
using System.Text.RegularExpressions; //Onyx edited
...
namespace Content.Client.Paper.UI
{
    ...
    public sealed partial class PaperWindow : BaseWindow
    {
        ...
        public void Populate(PaperComponent.PaperBoundUserInterfaceState state)
        {
            ...
            msg.AddMarkupPermissive(processed);
            // Starlight
        ...
        {
            // Remove sign_limit and sign_repeat_limit tags in both <> and [] forms
            text = Regex.Replace(text, "<\\s*sign_limit\\s*=\\s*\\d+\\s*>", string.Empty);
            text = Regex.Replace(text, "<\\s*sign_repeat_limit\\s*=\\s*\\d+\\s*>", string.Empty);
            text = Regex.Replace(text, "\\[\\s*sign_limit\\s*=\\s*\\d+\\s*\\]", string.Empty);
            text = Regex.Replace(text, "\\[\\s*sign_repeat_limit\\s*=\\s*\\d+\\s*\\]", string.Empty);
            return text;
        }

        private static string ReplaceSignaturePlaceholders(string text, System.Collections.Generic.List<Content.Shared._Onyx.Paper.SignatureDisplayInfo> signatures)
        {
            //Onyx added
            // Support <sign=id>, [sign=id], and bare <sign>, [sign], case-insensitive, with any id payload
            var underline = "[color=#000000]_____[/color]";

            var angleEq = new Regex("<\\s*sign\\s*=\\s*([^>]+)>", RegexOptions.IgnoreCase);
            ...
            var squareEq = new Regex("\\[\\s*sign\\s*=\\s*([^\\]]+)\\]", RegexOptions.IgnoreCase);
            ...
            var angleBare = new Regex("<\\s*sign\\s*>", RegexOptions.IgnoreCase);
            text = angleBare.Replace(text, _ => underline);

            var squareBare = new Regex("\\[\\s*sign\\s*\\]", RegexOptions.IgnoreCase);
            text = squareBare.Replace(text, _ => underline);
            //Onyx end
            return text;
        }
        //Onyx end
